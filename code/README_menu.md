...
`Calculate_Error()` 中的 `LOOKHEAD` 表示从图像底部向上的预瞻长度，而不是绝对结束行。图像高度为 120 时，近端截止行固定为第 90 行，因此 `LOOKHEAD=90/100/110` 分别从第 30/20/10 行开始计算。误差计算仍每隔 5 行采样，并按行使用平滑权重数列：远端、中端、近端默认锚点为 `0.40/1.00/1.50`，中间按两段线性插值，近端仍然保持更高权重。`VISION` 页面中的 `PATH FAR`、`PATH MIDDLE` 和 `PATH NEAR` 可以分别调节三个锚点。
...
直道 PD 使用 `STEERING KP/KD`，弯道 PD 使用 `CURVE KP/KD`。CPU0 对锁存的归一化曲率做一阶滤波，并使用 `CURVE ENTER` 和 `CURVE EXIT` 滞回切换；切换模式时会清零微分滤波状态，停车、重新启动和参数应用时也会重置模式。无效图像结果不会改变当前曲率模式。本次只调整 `image.c` 的误差预瞻范围和权重，不改变双 PD、速度控制和曲率字段的既有接口；图像端没有发布曲率时仍会保持直道 PD。
...
前瞻权重字段已经存在于 `car_params_t`，本次只启用默认值、菜单和误差计算，不改变结构体布局，也不升级 Flash 格式。读取旧档位时，如果三个前瞻权重全部为 0，会自动补入 `0.40/1.00/1.50`；负值会被限制为 0。保存和切换档位仍沿用 `SAVE FLASH`、`SWITCH GEAR`。
