...
按键每 10 ms 采样一次。短按在释放时执行，按住约 1 s 进入长按；上、下键长按后每 100 ms 重复调整，右、左和中心键的长按只执行一次。普通菜单空闲时不刷新 TFT，只有按键、参数操作或状态操作后刷新。
...
进入 `IMAGE VIEW` 后，原始图像和二值化图像最多每 50 ms 更新一次，并且只在收到新的处理结果后更新。`TELEMETRY` 页面每 50 ms 重新读取 `car_result`、帧状态和运行状态并刷新一次；其他列表页不会因为遥测变化而自动重绘。严格按键刷新模式下，代码直接修改 `car_params` 后，下一次按键或菜单操作会显示新值。

## TFT 和引脚

TFT 使用逐飞官方 `ips200` 硬件 SPI 接口：`ips200_set_dir(IPS200_CROSSWISE)` 和 `ips200_init(IPS200_TYPE_SPI)`。板级映射在 `board_pins.h`，显示屏使用 `SPI_2`：SCLK 为 `P15.3`，MOSI 为 `P15.5`，CS 为 `P15.2`，DC 为 `P15.0`，RESET 为 `P15.1`，背光为 `P20.14`。菜单按键均为低电平有效并配置为上拉输入。
